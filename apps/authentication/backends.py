...
from .models import User,DashBoard,Shop
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        auth_header_prefix2 = self.authentication_header_prefix2.lower()
        if not auth_header:
            return None
        ...
        elif prefix.lower() == auth_header_prefix2:
            try:
                dashboard=DashBoard.objects.get(token=token)
                if not dashboard.is_active:
                    return None
    ...
    def _authenticate_shops(self, request, token):
        ...
        if not shop.status=="actived":
            msg = 'This shop has not been activated.'
            raise exceptions.AuthenticationFailed(msg)
